...
def _validate_puzzle_part(part: Dict[str, Any], part_number: int) -> List[str]:
    ...
    elif not isinstance(part['rows'], str):
        errors.append(f"Part {part_number} (puzzle): 'rows' must be a string, got {type(part['rows']).__name__}")
    
    if 'columns' not in part:
        errors.append(f"Part {part_number} (puzzle): Missing 'columns'")
    elif not isinstance(part['columns'], str):
        errors.append(f"Part {part_number} (puzzle): 'columns' must be a string, got {type(part['columns']).__name__}")
