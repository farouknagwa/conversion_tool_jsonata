...
from SCRIPTS.utils import (
    ...
    extract_country_code_mandatory_return,
    is_empty_or_none,
)
...
def validate_json_structure(json_data: Dict[str, Any]) -> Tuple[List[str], List[str]]:
    ...
    if len(json_data.get("parts", [])) > 1 and is_empty_or_none(
        json_data.get("statement")
    ):
...
def _validate_part(
    part: Dict[str, Any], part_number: int, country_code: str
...
def _validate_choice(
    choice: Dict[str, Any], choice_index: int, part_number: int, part_type: str
) -> List[str]:
    """
    ...
    required_fields = ["type", "html_content", "values", "unit", "index", "fixed_order"]
    ...
    if (
        part_type in ["mcq", "mrq", "gmrq"]
        and choice.get("type") not in VALID_CHOICE_TYPES
    ):
        errors.append(
    ...
    if (
        not isinstance(choice.get("html_content"), str)
        or not choice.get("html_content", "").strip()
    ):
        errors.append(
...
def _validate_mrq(
    ...
    if len(key_choices) < 1:
        errors.append(
            f"Part {part_number} (mrq): Must have at least 1 key choice, found {len(key_choices)}"
        )
...
def _validate_frq_ai(part: Dict[str, Any], part_number: int) -> List[str]:
    ...
    if not isinstance(part.get("choices"), list):
        errors.append(f"Part {part_number} (frq_ai): 'choices' must be an empty array")
...
def _validate_oq(part: Dict[str, Any], part_number: int) -> Tuple[List[str], List[str]]:
    ...
    if (
        is_empty_or_none(part.get("direction"))
        or part.get("direction", "").lower() not in VALID_DIRECTIONS
    ):
        warnings.append(
...
def _validate_gap_text(part: Dict[str, Any], part_number: int) -> List[str]:
    ...
    if not isinstance(part.get("choices"), list):
        ...
        errors.append(
            f"Part {part_number} (gapText): 'choices' must be empty for gapText"
        )

    # Validate gap_text_keys
    if (
        not isinstance(part.get("gap_text_keys"), list)
        or len(part.get("gap_text_keys", [])) == 0
    ):
        errors.append(
    ...
    if stem:
        ...
        valid_gaps = [
            gap
            for gap in gap_spans
            if gap.parent
            and gap.parent.name
            in ["p", "div", "span", "body", "ul", "ol", "li", "td", "th"]
        ]
    ...
    if stem_gaps_count == 0:
        errors.append(f"Part {part_number} (gapText): stem must have at least one gap.")
    else:
        ...
        correct_gapped_text_keys_count = len(
            [k for k in part["gap_text_keys"] if "correct_order" in k]
        )
...
def _validate_string(part: Dict[str, Any], part_number: int) -> List[str]:
    ...
    if part.get("ai", {}).get("guidelines") is not None:
        ...
        if not isinstance(guidelines, list):
            errors.append(f"Part {part_number} (string): 'guidelines' must be an array")
        else:
...
def _validate_matching(part: Dict[str, Any], part_number: int) -> List[str]:
    ...
    # validate that the choices in each group are equal in length
    group1_choices = [c for c in part["choices"] if c.get("group") == 1]
    group2_choices = [c for c in part["choices"] if c.get("group") == 2]
    if len(group1_choices) != len(group2_choices):
        errors.append(
            f"Part {part_number} (matching): Groups must be equal in length, found {len(group1_choices)} and {len(group2_choices)}"
        )
...
def _validate_gmrq(part: Dict[str, Any], part_number: int) -> List[str]:
    ...
    # Validate choices array
    if not isinstance(part.get("choices"), list) or len(part.get("choices", [])) == 0:
        errors.append(f"Part {part_number} (gmrq): 'choices' must be a non-empty array")
        return errors
    ...
    if len(group1_choices) != len(group2_choices):
        errors.append(
            f"Part {part_number} (gmrq): Groups must be equal in length, found {len(group1_choices)} and {len(group2_choices)}"
        )
...
def _validate_counting(part: Dict[str, Any], part_number: int) -> List[str]:
    ...
    if (
        not grid_size
        or not isinstance(grid_size, str)
        or not re.match(GRID_SIZE_PATTERN, grid_size)
    ):
        errors.append(
...
def _validate_puzzle(part: Dict[str, Any], part_number: int) -> List[str]:
    ...
    # Validate choices array
    if not isinstance(part.get("choices"), list):
        errors.append(f"Part {part_number} (puzzle): 'choices' must be an empty array")
    elif len(part.get("choices", [])) > 0:
        errors.append(
    ...
    if part.get("puzzleImageHeight") and not re.match(
        r"^\d+$", str(part["puzzleImageHeight"])
    ):
        errors.append(
            f"Part {part_number} (puzzle): 'puzzleImageHeight' must be a string representing a number"
        )

    if part.get("puzzleImageWidth") and not re.match(
        r"^\d+$", str(part["puzzleImageWidth"])
    ):
        errors.append(
    ...
    if (
        not isinstance(part.get("puzzleImageSplited"), list)
        or len(part.get("puzzleImageSplited", [])) == 0
    ):
        errors.append(
        ...
        for i, piece in enumerate(part["puzzleImageSplited"]):
            ...
            if "correct_order" not in piece or not isinstance(
                piece["correct_order"], int
            ):
                errors.append(
...
def _validate_root_answer_for_explanation(
    ...
    if not is_empty_or_none(explanation) and isinstance(explanation, str):
        try:
            ...
            if (
                number_of_parts > 1
            ):  # For multipart: must be one parent <div> with direct child <div>s equal to number_of_parts
                # Get all direct children of the soup
                direct_children = [
                    tag for tag in soup.children if tag.name is not None
                ]  # Get soup direct children
                if len(direct_children) != 1:
                    ...
                    child_divs = [
                        tag for tag in parent_div.children if tag.name == "div"
                    ]
...
def validate_pre_conversion(
    json_data: Dict[str, Any], filename: str
